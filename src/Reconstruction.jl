...
function generate_dpList(
    ...
    elseif mode == "frc"
        if mod(length(dps), 2) == 0
            ...
            count = 1
            for ii = 1:size(dps)[3]
                dpList[count:count-1+size(dps)[4]/2] =
                    ((ii-1)*size(dps)[4].+1:2:ii*size(dps)[4]) .+ mod(ii + OtherPara[1], 2)
    ...
    elseif mode == "columns"
        ...
        OtherPara = Int.(OtherPara)
        for ii in OtherPara
            dpList[count:size(dps)[3]+count-1] =
                (ii:size(dps)[4]:ii+(size(dps)[3]-1)*size(dps)[4])'
    ...
    elseif mode == "subarray"
        ...
        dpList =
            zeros((OtherPara[4] - OtherPara[3] + 1) * (OtherPara[2] - OtherPara[1] + 1), 1)
        count = 0
...
function prestart(params::Parameters, dps::DiffractionPatterns)
    ...
    dpList, trans_related, List_index =
        generate_dpList(dps, trans_related, 1, "double-skip", 2)
...
function iterate(
    ...
    for count_dp = 1:length(List_index)
        sx =
            round(Int, trans_exec[List_index[count_dp], 1]):round(
                Int,
                trans_exec[List_index[count_dp], 1] + size(dps)[1] - 1,
            )
        sy =
            round(Int, trans_exec[List_index[count_dp], 2]):round(
                Int,
                trans_exec[List_index[count_dp], 2] + size(dps)[2] - 1,
            )
        ...
        probe =
            probe .+
            params.ProbeUpdate .* (ew1 .- ew) .* conj.(obj[sx, sy]) ./
            maximum(abs.(obj[sx, sy]) .^ 2)
        obj[sx, sy] =
            obj[sx, sy] .+
            params.ObjUpdate .* (ew1 .- ew) .* conj.(probe0) ./ maximum(abs.(probe0) .^ 2)
